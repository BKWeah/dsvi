-- Clean TODO tracking migration only
-- Date: 2025-05-29
-- Purpose: Add TODO tracking system without modifying auth tables

-- Create project_tasks table for TODO tracking
...
CREATE INDEX IF NOT EXISTS idx_project_tasks_approved ON project_tasks(approved);
CREATE INDEX IF NOT EXISTS idx_project_task_audit_task_id ON project_task_audit(task_id);
...
-- Public read access (clients can view progress)
CREATE POLICY "project_tasks_public_read" ON project_tasks 
FOR SELECT USING (true);

-- Authenticated users can modify (for now - can be restricted later)
...
CREATE POLICY "project_task_audit_authenticated_read" ON project_task_audit 
FOR SELECT USING (auth.role() = 'authenticated');

-- System can insert audit logs
CREATE POLICY "project_task_audit_system_insert" ON project_task_audit 
FOR INSERT WITH CHECK (true);

-- Insert initial 130-hour MVP tasks
...
SELECT 'TODO tracking system added successfully!' as status;